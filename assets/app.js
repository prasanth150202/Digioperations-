...
async function genPPTX() {
  ...
  try {
    ...
    s1.addShape('rect', { x: 0.4, y: 5.0, w: 3.0, h: 1.4, fill: { color: 'FFFFFF' }, rectRadius: 0.1, line: { color: COLOR_AMBER, width: 2 } });
    ...
    s2.addText(`Born in Coimbatore. Built for India.`, {x: 0.7, y: 2.1, fontSize: 20, bold: true, color: COLOR_DARK, fontFace: FONT_PRIMARY, w: 11.0, h: 0.35});
    s2.addText(`Mr. ${fName} quit a high-paying corporate job to solve one overlooked problem: international sizing and products are not engineered for Indian body proportions.\n\nOur products are purpose-built for Indian proportions, chest widths, and thigh circumferences — so every workout feels right.`, { x: 0.7, y: 2.7, w: 5.4, fontSize: 12, color: '333333', fontFace: FONT_PRIMARY });
    ...
    // Right Stack of 5 features
    const usps = [
      { t: 'Anti-Rip Technology', d: 'Engineered to survive the most brutal workouts — every rep, every set.', c: COLOR_PRIMARY },
      { t: 'Odour-Free Fabric', d: 'Stay fresh through every session. Multi-hour wear, zero compromise.', c: COLOR_AMBER },
      { t: 'Sweat-Wicking', d: 'Moisture pulled away instantly — keeps you dry and fully focused.', c: COLOR_RED },
      { t: '4-Way Ultra Stretch', d: 'Zero restriction in any direction. Full range. No pulls, no tears.', c: COLOR_PURPLE },
      { t: 'Indian Body Fit', d: 'Built for Indian proportions — not scaled down from global sizes.', c: COLOR_GREEN }
    ];
    usps.forEach((u, i) => {
    ...
    s3.addText(`"Build the Gymshark of India — where every serious lifter wears ${bName} not because of an ad, but because everyone at the gym already does."`, { x: 0.5, y: 1.8, w: 12.3, fontSize: 13, color: COLOR_AMBER, italic: true, fontFace: FONT_PRIMARY });
    
    // 4 Grid blocks
    const vision = [
      { t: 'COMMUNITY IDENTITY', d: '"I lift. I wear Black Ape." Own the identity of the serious Indian gym community. Be the brand serious lifters call their own.', c: COLOR_PRIMARY },
      { t: 'PERFORMANCE TRUST', d: 'Products that prove themselves in every rep. No fluff — just fabric tech, body fit, and real gym performance.', c: COLOR_AMBER },
      { t: 'DARK AESTHETIC', d: 'THE Indian brand for serious, performance-focused dark content. Own this aesthetic before anyone else does.', c: COLOR_GREEN },
      { t: 'STORY > PROMOTION', d: 'Indian body. Indian founder. Indian ambition. This story beats any competitor discount every single time.', c: COLOR_PURPLE }
    ];
    vision.forEach((v, i) => {
      let x = 0.5 + (i % 2) * 6.2;
    ...
    const pNames = [stratForm['pname0'] || 'THE IRON MONK', stratForm['pname1'] || 'THE AESTHETIC CHASER', stratForm['pname2'] || 'THE MOTIVATED BEGINNER'];
    ...
    const pPains = [stratForm['ppain0'] || 'Gym is identity. Clothes signal seriousness. Won\'t compromise on fit.', stratForm['ppain1'] || 'Performance AND style. Wants to look good inside and outside the gym.', stratForm['ppain2'] || 'Wants to feel like they belong. Right gear = motivation to show up.'];
    const pMsgs = ['"Built to show what you\'ve built"', '"Sculpted. Strong. Unstoppable."', '"Start somewhere. Look like you belong."'];
    ...
    for (let i = 0; i < 3; i++) {
      ...
      s4.addText(i === 0 ? 'Stringers · Muscle Fits · Joggers' : i === 1 ? 'Oversized Tees · Shorts · Leggings' : 'Oversized Tee · Shorts', {x: x + 0.2, y: 4.1, w: 3.5, fontSize: 11, color: COLOR_DARK, fontFace: FONT_PRIMARY, h: 0.35});
    ...
    addHeader(s5, 'Market Research', 'India Activewear Market — The ₹55,000 Crore Opportunity');
    
    // Top 4 cards
    const mkt = [
      { v: '₹55,000 Cr', l: 'India Activewear Market 2025', c: COLOR_PRIMARY },
      { v: '38%', l: 'YoY D2C Gymwear Growth Rate', c: COLOR_GREEN },
      { v: '50M+', l: 'Active Gym Members India', c: COLOR_PURPLE },
    ...
    s5.addText('TAM: ₹55,000 Cr — Total India activewear retail\nSAM: ₹8,000–10,000 Cr — Premium D2C gymwear\nSOM: ₹3–5 Cr ARR — Realistic 12-month target\nGAP: OPEN FIELD — No Indian brand owns body-fit story.', { x: 0.7, y: 3.2, w: 5.5, fontSize: 12, lineSpacing: 26, color: COLOR_DARK, fontFace: FONT_PRIMARY });
    ...
    s5.addText('· Dark Aesthetic: Gymshark effect reaching serious Indian lifters.\n· Gym = Identity: Consumers wear gym brands 24/7 as lifter badge.\n· Creator Commerce: 62% buy based on creator recommendation.\n· Indian Body Gap: Global brands do not fit Indian ratios.', {x: 7.1, y: 3.2, w: 5.5, fontSize: 12, lineSpacing: 26, color: COLOR_DARK, fontFace: FONT_PRIMARY, h: 0.35});
    ...
    let tableData = [
      [
        ...
        { text: 'BLACK APE WINS', options: { fill: COLOR_DARK, color: 'FFFFFF', bold: true, align: 'center', fontSize: 10 } }
      ],
      ...
      ['Gymshark', '₹3,000+', 'NO', 'BENCHMARK', 'Not in India yet. Global sizing.', 'Own India before they enter']
    ];
    ...
    s6.addText('KEY INSIGHT: No Indian brand currently owns the \'dark, serious lifter\' aesthetic + Indian body-fit story combined. This is our unclaimed territory.', { x: 0.7, y: 6.3, w: 11.9, fontSize: 11, bold: true, color: 'FFFFFF', fontFace: FONT_PRIMARY });
    ...
    const funnel = [
      { id: '1', s: 'AWARENESS', t: 'Day 0', ch: 'Meta Broad · YouTube · Influencer', m: 'Product feature story — show the tech', a: 'Dark performance videos', c: COLOR_PRIMARY },
      ...
      { id: '7', s: 'ADVOCACY', t: 'Day 60+', ch: 'Instagram UGC · Community · Referral', m: '"I am Black Ape" identity content', a: 'UGC reposts, community events', c: COLOR_RED }
    ];
    ...
    addHeader(s10, 'Meta Ads Strategy', 'Facebook & Instagram — ₹1,10,000/Month Deep Dive');
    const metaCols = [
      { t: 'TOFU (Awareness)', amt: '₹45,000', aud: 'Fitness interests · Gym · Bodybuilding · Pan India', fmt: '15-30 sec Video Reels · Dark aesthetic styling', cr: 'Product features + tech demo reels', k: 'CPM < ₹80 · Video View Rate 25%+', c: COLOR_PRIMARY },
      { t: 'MOFU (Nurture)', amt: '₹30,000', aud: 'Video viewers 50%+ · IG Engagers · Site visitors 30d', fmt: 'Carousel Ads · Collection Ads · Slideshows', cr: 'Features + reviews + story + unboxings', k: 'CTR 1.5%+ · Add-to-Cart Rate 4%+', c: COLOR_PURPLE },
      { t: 'BOFU (Convert)', amt: '₹35,000', aud: 'Add to cart + checkout abandoners · 7-day visitors', fmt: 'Single Product Ads · Dynamic Product Ads (DPA)', cr: 'Product + free shaker at ₹1,499+ offer', k: 'ROAS 4x+ · CPA < ₹400 limit', c: COLOR_GREEN }
    ];
    ...
    s10.addText('5 CREATIVE ANGLES TO TEST IN PARALLEL: Feature Explainer · Transformation · Dark Brand Film · Raw UGC · Influencer Cut', {x: 0.7, y: 6.35, w: 11.9, fontSize: 10, bold: true, color: 'FFFFFF', fontFace: FONT_PRIMARY, h: 0.35});

    // SLIDE 11: Google & YouTube (Light Theme)
    let s11 = pptx.addSlide();
    addHeader(s11, 'Google & YouTube', 'Intent Capture + Visual Brand Building — ₹1,10,000/Month');
    ...
    s11.addText('GOOGLE ADS — ₹70,000/Month', { x: 0.7, y: 1.25, fontSize: 14, bold: true, color: 'FFFFFF', fontFace: FONT_PRIMARY });
    s11.addText('Shopping Campaigns — ₹40,000\n· All 8 hero SKUs with SEO-optimised titles.\n· Format: "Black Ape [Product] — Squat Proof | Anti-Rip | India".\n· Keywords: "gym shorts India", "squat proof leggings".\n\nSearch Campaigns — ₹30,000\n· Brand Bid: "Black Ape Apparel" — brand protection.\n· Competitor conquests: "Fuaark alternatives", "GymX joggers".\n· Non-brand: "best gymwear India", "performance gym shorts".', { x: 0.7, y: 1.9, w: 5.5, fontSize: 11, lineSpacing: 20, color: COLOR_DARK, fontFace: FONT_PRIMARY });
    s11.addShape('rect', { x: 0.7, y: 4.8, w: 5.5, h: 1.6, fill: { color: COLOR_DARK }, rectRadius: 0.05 });
    s11.addText('SAMPLE AD SCRIPT (15-sec)\n[Heavy squat] "Your shorts just ripped. Again." [Cut to Black Ape Shorts] "Squat Proof. Anti-Rip. Built for India." [CTA: Shop Now — ₹999]', {x: 0.8, y: 4.95, w: 5.3, fontSize: 10, italic: true, color: 'FFFFFF', fontFace: FONT_PRIMARY, h: 0.35});
    ...
    s11.addText('YOUTUBE ADS — ₹40,000/Month', { x: 7.1, y: 1.25, fontSize: 14, bold: true, color: 'FFFFFF', fontFace: FONT_PRIMARY });
    s11.addText('6-Second Bumper Ads — ₹15,000\n· Dark visual + product name + single hook. Unskippable.\n· "Anti-rip. Squat proof. Built for India." — mass impression at scale.\n\n15–30 Sec Pre-Roll — ₹25,000\n· Structure: Problem → Feature → Proof → CTA.\n· Target: Fitness channels, bodybuilding tutorials, gym vlogs.\n\nTarget Channels:\n· Fitness YouTube & Bodybuilding channels\n· Gym tutorials & transformation vlogs\n· Supplement review channels', { x: 7.1, y: 1.9, w: 5.5, fontSize: 11, lineSpacing: 20, color: COLOR_DARK, fontFace: FONT_PRIMARY });

    // SLIDE 12: Influencer Strategy (Light Theme)
    let s12 = pptx.addSlide();
    addHeader(s12, 'Influencer Strategy', 'Creator Omnipresence — ₹50,000/Month, Maximum Impact');
    s12.addShape('rect', { x: 0.5, y: 1.1, w: 12.3, h: 0.6, fill: { color: COLOR_DARK } });
    s12.addText('THE OMNIPRESENCE GOAL: When someone opens fitness Instagram in India, they see Black Ape on 3+ different creators within a week. Not viral — just everywhere.', { x: 0.7, y: 1.25, w: 11.9, fontSize: 10, bold: true, color: COLOR_AMBER, fontFace: FONT_PRIMARY });

    const infCols = [
      { t: 'MICRO CREATORS', amt: '8 Creators · ₹28,000', w: '10K–50K Followers', y: 'Highest engagement rates (8–12%). Niche gym audiences. Most authentic content. Direct community trust.', d: '1 REEL — Product in ACTUAL workout. Show stretch, fit, tech. Dark gym aesthetic.\n2 STORIES — Unboxing reaction + "gym look of the day".\n1 STATIC POST — Clean product shot + feature-led caption.', c: COLOR_PRIMARY },
      { t: 'MID-MACRO CREATOR', amt: '1 Creator · ₹20,000', w: '100K–500K Followers', y: 'Broader reach for brand authority. Signals we are a real, fast-growing premium D2C brand.', d: '1 REEL — Full workout in our gear, authentic feature review.\nBroad awareness reach targeting serious Indian lifters.\nUsage rights: 90 days for repurposing as paid Meta ads.', c: COLOR_PURPLE },
      { t: 'USAGE RIGHTS', amt: 'All · ₹2,000', w: 'All Creators', y: 'Repurpose the best creator content as paid Meta ads. Creator UGC = top-performing ad creative.', d: 'Repurpose rights: 90 days for paid ads across Meta.\nEvery creator brief must include usage rights clause.\nBest UGC content becomes BOFU retargeting ads.', c: COLOR_GREEN }
    ];
    ...
    const socCols = [
      { t: 'INSTAGRAM', s: '@blackapeindia', c: '3x Reels/week: Product features · Athlete workouts · Dark lifestyle\n2x Static Posts: Product photography · Customer UGC\n5x Stories/day: Polls · Gym tips · Product of the day\n1x Carousel/week: Product deep-dive feature explainer', id: 'Feed: Dark, moody, high-contrast. Black + navy + blue. NO studio-white. Caption tone: Bold. Direct.', tag: '#BlackApe #BuiltForIndia #GymwearIndia', color: COLOR_RED },
      { t: 'YOUTUBE', s: 'Black Ape Apparel', c: '2x Long-form/month: Gymwear comparisons, product deep-dives\nFounder Story: "Why Mr. Riyas quit his job to build Black Ape"\nGym performance: "Full Leg Day in Black Ape Joggers — Does It Hold?"\nYouTube Shorts from long-form content (zero extra effort)', id: 'Dark thumbnails. Strong physique visuals. Bold white text on black bg. SEO: "best gymwear India 2025".', tag: 'Monthly founder LIVE: Product drops + Q&A session', color: 'FF0000' },
      { t: 'FACEBOOK', s: 'blackapeindia', c: 'Primary use: Paid retargeting pixel + dynamic product ads\nCommunity Group: "Black Ape Lifters" — drop announcements + Q&As\nCatalogue connected for dynamic product retargeting ads\nNot primary organic channel — mainly used for paid retargeting', id: 'Focus: pixel-perfect attribution + catalogue for dynamic ads. Community group for loyal buyers.', tag: 'Secondary organic — primary paid retargeting + community', color: COLOR_PRIMARY }
    ];
    ...
    s14.addText('Muscle Fit\n4-way stretch + biceps strap. Fits like it was made for your body.\n\nStringers\nNon-restrictive. Odour-free. Built for serious sets.\n\nLeggings\nSpider Web waistband. Built-in underwear. Squat proof.\n\nOversized Tee\nSweat-wicking. Anti-odour. Hides insecurities. 60kg or 100kg — it works.', {x: 0.7, y: 2.5, w: 5.5, fontSize: 10, bold: true, color: COLOR_DARK, fontFace: FONT_PRIMARY, h: 0.35});
    ...
    const retCols = [
      { t: 'WHATSAPP', s: 'Business API · 85%+ Open Rate', c: 'Order Confirmed\n"Your Black Ape order is confirmed! [Order summary]"\n\nDay 3–5\n"Your gear arrived! Care tip + styling tip for your next session"\n\nDay 14\n"How\'s your first week? Tag us @blackapeindia"\n\nDay 30\n"New [Product] just dropped — check it out! [Link]"\n\nDay 45\n"Complete your kit — here\'s what pairs with your [product]"', color: COLOR_GREEN },
      { t: 'EMAIL', s: 'Email CRM · 28%+ Open Rate', c: 'Day 0 Welcome\nBrand story + founder origin + what we stand for\n\nDay 2\n"The technology inside your [product name]" — deep features\n\nDay 5\n"Real lifters. Real results." — social proof + UGC + reviews\n\nDay 8\n"Your exclusive offer — because you\'re part of the family"\n\nDay 10\n"What\'s your next lift? Here\'s what pairs with your [product]"', color: COLOR_PRIMARY },
      { t: 'PUSH NOTIFICATIONS', s: 'Pushnova · 15%+ CTR', c: 'Flash Sale\n"48 hours only — [Product] now ₹X. Free shaker at ₹1,499"\n\nBack In Stock\n"[Product] in your size is back! Limited stock — grab it"\n\nSocial Proof\n"47 people bought this today — your size is still available"\n\nCart Expires\n"You left something behind Your cart expires in 2 hours"\n\nNew Drop\n"NEW DROP: [Product] just launched. Be the first to get it"', color: COLOR_PURPLE }
    ];

    retCols.forEach((col, i) => {
      let x = 0.5 + (i * 4.25);
      s15.addShape('rect', { x, y: 1.1, w: 3.9, h: 5.6, fill: { color: 'FFFFFF' }, rectRadius: 0.1, line: { color: col.color, width: 2 } });
    ...
    const croPanels = [
      { t: 'HOMEPAGE', c: '· Dark full-width hero video: Real athletes, gym footage, in motion.\n· USP strip: Anti-Rip · Squat Proof · Indian Body Fit · Free Shipping.\n· UGC wall: Customer gym photos + creator feed at bottom.\n· Trust bar: "10,000+ Happy Lifters" · Star ratings.', color: COLOR_PRIMARY },
      { t: 'PRODUCT PAGES', c: '· Headline leads with FEATURE — not just product name.\n· 6–8 images: Dark model shots + flat lay + fabric close-up.\n· 15-sec product video: Show the stretch test, anti-rip test.\n· Feature pills displayed ABOVE the fold — not buried in description.', color: COLOR_PURPLE },
      { t: 'CART & CHECKOUT', c: '· Progress bar: "Add ₹X more for FREE Shaker + Free Shipping".\n· Cross-sell: "People who bought Joggers also got Tank Top".\n· PREPAID50 code prominently shown at checkout page.\n· Trust badges: Secure payment + easy returns + Made in India.', color: COLOR_AMBER },
    ...
    croPanels.forEach((p, i) => {
      ...
      s16.addShape('rect', { x, y, w: 5.9, h: 0.45, fill: { color: p.color } });
    ...
    const kpis = [
      ...
      { v: '< ₹450', l: 'Customer Acquisition Cost', d: 'If CAC > ₹450 on ₹1,500 AOV, margin suffers', c: COLOR_AMBER },
    ...
    addHeader(s18, 'Revenue Projections', 'Month-by-Month Revenue Forecast — ₹2L to ₹30L in 90 Days');
    
    // Month cards
    const mCards = [
      { m: 'M0 (NOW)', v: mRev, o: '~133 orders · AOV ₹1,500', d: 'Baseline — pre-strategy', c: COLOR_PRIMARY },
      { m: 'MONTH 1', v: '₹6L', o: '~375 orders · AOV ₹1,600', d: 'All channels live + seeding', c: COLOR_PRIMARY },
      { m: 'MONTH 2', v: '₹15L', o: '~882 orders · AOV ₹1,700', d: 'Scale winners + retention', c: COLOR_GREEN },
    ...
    s18.addText('Meta Ads: 40% — ₹12,00,000\nGoogle+Search: 25% — ₹7,50,000\nInfluencer: 15% — ₹4,50,000\nRetention: 15% — ₹4,50,000\nOrganic/Direct: 5% — ₹1,50,000\n\nMonthly Spend: ₹3,00,000 · Blended ROAS: 10x · Gross Margin (40%): ₹12,00,000', { x: 7.1, y: 3.5, w: 5.5, fontSize: 11, lineSpacing: 22, color: COLOR_DARK, fontFace: FONT_PRIMARY });
    ...
    addDarkHeader(s20, 'Cult Strategy', 'THE GYMSHARK OF INDIA. THE LONG GAME.');
    ...
    s20.addText('"Gymshark went from £0 to £250 Million in 8 years — through community, not discounts. This is our playbook."', { x: 0.5, y: 1.8, w: 12.3, fontSize: 13, color: COLOR_AMBER, italic: true, fontFace: FONT_PRIMARY });

    const cult = [
      { t: 'CREATOR ARMY', d: '50+ creators wearing Black Ape creates omnipresence — not viral, just everywhere. Every serious Indian gym creator has worn and reviewed Black Ape. UGC pipeline never runs dry. CPMs drop as organic grows.', c: COLOR_PRIMARY },
      { t: 'PRODUCT DROP CULTURE', d: 'Scarcity creates desire. Desire creates community. Limited drops with waitlists. Early access for loyal buyers. "Sold Out" is a marketing event. People beg to be on the next launch list.', c: COLOR_AMBER },
      { t: 'COMMUNITY IDENTITY', d: '"I lift. I wear Black Ape. That\'s who I am." Black Ape Athletes program. Community challenges: "Show your lift in Black Ape." WhatsApp community for top 500 buyers. Identity over discounts, always.', c: COLOR_GREEN },
      { t: 'STORY > PROMOTION', d: 'Indian body, underserved for decades. Mr. Riyas quit his job to fix it. Made in India. Built for India. Every content piece reinforces identity — never a discount. This narrative is undefeatable.', c: COLOR_PURPLE }
    ];
    cult.forEach((v, i) => {
      let x = 0.5 + (i % 2) * 6.2;
      let y = 2.4 + Math.floor(i / 2) * 2.1;
      s20.addShape('rect', { x, y, w: 5.9, h: 1.8, fill: { color: COLOR_DARK2 }, rectRadius: 0.05, line: { color: v.c, width: 2 } });
    ...
    let techTable = [
      [
        ...
        { text: 'FUAARK', options: { fill: COLOR_DARK, color: 'FFFFFF', bold: true, align: 'center', fontSize: 10 } },
        { text: 'GYMX', options: { fill: COLOR_DARK, color: 'FFFFFF', bold: true, align: 'center', fontSize: 10 } },
        { text: 'GYMSHARK BENCHMARK', options: { fill: COLOR_DARK, color: 'FFFFFF', bold: true, align: 'center', fontSize: 10 } },
        { text: 'BLACK APE BUILD', options: { fill: COLOR_DARK, color: 'FFFFFF', bold: true, align: 'center', fontSize: 10 } }
      ],
      ...
      ['Loyalty', 'Fuaark Clan Coins', 'None', 'Gymshark XP', 'Growave / Smile.io'],
      ['Mobile App', 'Yes (Appbrew)', 'No', 'Yes (Custom)', 'Phase 2 – Month 3'],
      ['Upsell', 'Not confirmed', 'Beam OutSell', 'Rebuy', 'Rebuy / AfterSell'],
      ['Push Notifications', 'Not confirmed', 'Not confirmed', 'Yes', 'PushOwl'],
      ['Ad Channels', 'Meta (FB/IG)', 'Meta+Google+Snap', 'Meta+Google+YT', 'Meta+Google+YT']
    ];
    ...
    s21.addText('Fuaark Clan Coins = loyalty threat. Counter with WATI automation + Klaviyo flows — out-retain them, not out-discount them.', { x: 0.7, y: 6.3, w: 11.9, fontSize: 11, bold: true, color: 'FFFFFF', fontFace: FONT_PRIMARY });
    ...
    addHeader(s24, 'Tech Stack', 'Black Ape Recommended Tech Stack — Tools to Install');
    const techStack = [
      { t: 'ZingBot', amt: '₹4,000–7,000/mo', sub: 'DAY 1', desc: 'WhatsApp Business API — automated order confirm, cart recovery, broadcasts.\n\nFuaark has NO automation. GymX has NO automation. Immediate advantage.', color: COLOR_GREEN },
      { t: 'Email Automation', amt: 'Free → ₹3,500/mo', sub: 'DAY 1', desc: 'Full email CRM — 5-email welcome flow, abandoned cart, post-purchase sequences.\n\nNeither Fuaark nor GymX uses email marketing. 0% competition in inbox.', color: COLOR_PRIMARY },
      { t: 'Judge.me', amt: '₹1,500–2,500/mo', sub: 'DAY 1', desc: 'Photo + video reviews auto-collected. Feeds Google Shopping + builds trust wall.\n\nFuaark uses Judge.me. Match them. Add photo incentive for UGC advantage.', color: COLOR_RED },
      { t: 'PushNova', amt: 'Free → ₹1,200/mo', sub: 'MONTH 1', desc: 'Web push notifications — flash sale alerts, back-in-stock, abandoned cart.\n\nNeither competitor uses push. 15%+ CTR on engaged subscribers.', color: COLOR_PURPLE },
      { t: 'Rebuy / AfterSell', amt: '₹2,500–4,000/mo', sub: 'MONTH 1', desc: 'Post-purchase upsell page + cart upsells. "Complete the outfit" cross-sells.\n\nGymX has Beam OutSell. Match + exceed with Rebuy\'s AI engine.', color: COLOR_AMBER },
    ...
    techStack.forEach((stack, i) => {
      ...
      s24.addShape('rect', { x, y, w: 3.9, h: 2.5, fill: { color: 'FFFFFF' }, rectRadius: 0.1, line: { color: stack.color, width: 2 } });
      s24.addShape('rect', { x, y, w: 3.9, h: 0.45, fill: { color: stack.color } });
    ...
    const gaps = [
      { t: 'WhatsApp Automation', d: 'THEM: Fuaark = manual · GymX = broadcast only\nUS: WATI automated flows — 8 touchpoints from Day 1\nRecover 30–40% of abandoned carts competitors lose forever', c: COLOR_GREEN },
      { t: 'Email Marketing', d: 'THEM: Neither Fuaark nor GymX runs email nurture\nUS: Klaviyo 5-email welcome flow + lifecycle sequences\nZero competition in inbox = 25–30% open rate from Day 1', c: COLOR_PRIMARY },
      { t: 'YouTube Strategy', d: 'THEM: No Indian gymwear brand owns YouTube SEO\nUS: 2 videos/month — size guide, feature demos, founder story\nCompound organic traffic — GymX not there, Fuaark barely', c: COLOR_RED },
      { t: 'Google Shopping', d: 'THEM: GymX runs Shopping. Fuaark does NOT.\nUS: All 8 SKUs with SEO-optimised titles from Day 1\nCapture purchase-intent searches Fuaark ignores completely', c: COLOR_AMBER },
      { t: 'Premium Offer Design', d: 'THEM: Both brands = discount-dependent. Codes everywhere.\nUS: Gift-first strategy — shaker, whey, early access. No slashing.\nHigher AOV, better margins, loyal customers who pay full price', c: COLOR_PURPLE },
      { t: 'Indian Body Story', d: 'THEM: No competitor owns the \'engineered for India\' narrative\nUS: Every ad, page, and post leads with Indian body fit identity\nEmotional moat — once owned, impossible for others to steal', c: COLOR_PRIMARY }
    ];

    gaps.forEach((gap, i) => {
      let x = 0.5 + (i % 3) * 4.25;
      let y = 1.1 + Math.floor(i / 3) * 2.8;
      s25.addShape('rect', { x, y, w: 3.9, h: 2.5, fill: { color: COLOR_DARK2 }, rectRadius: 0.1, line: { color: gap.c, width: 2 } });
    ...
    calCols.forEach((col, i) => {
      ...
      s27.addShape('rect', { x, y: 1.1, w: 3.9, h: 0.5, fill: { color: col.color } });
    ...
    s27.addText('★ Priority: World Health Day · Tamil New Year · Mother\'s Day · Father\'s Day · Yoga Day (Jun 21) — plan creatives 2 weeks in advance', {x: 0.7, y: 6.35, w: 11.9, fontSize: 10, bold: true, color: 'FFFFFF', fontFace: FONT_PRIMARY, h: 0.35});

    // SLIDE 28: Brand Ambassadors (Light Theme)
    let s28 = pptx.addSlide();
    addHeader(s28, 'Brand Ambassadors', 'Black Ape Athletes Program — Building the Cult Creator Army');
    s28.addShape('rect', { x: 0.5, y: 1.1, w: 12.3, h: 0.5, fill: { color: COLOR_DARK } });
    s28.addText('TARGET: 50+ creators wearing Black Ape by Month 6. Every serious Indian gym creator has worn, reviewed, and tagged us.', { x: 0.7, y: 1.25, w: 11.9, fontSize: 10, bold: true, color: COLOR_AMBER, fontFace: FONT_PRIMARY });

    const ambCols = [
      { t: 'PHASE 1 — MONTH 1', sub: '15 Micro-Creators', c: '· 10K–50K followers, active gym-goers.\n· Dark/moody aesthetic required — no studio-white.\n· Deliverable: 1 Reel + 2 Stories + 1 Static.\n· Usage rights: 90 days for paid ads.\n· Budget: ₹3,000–4,000 per creator (₹45,000 total).\n· Track: engagement rate, saves, DM volume.', color: COLOR_PRIMARY },
      { t: 'PHASE 2 — MONTH 2', sub: '10 More Micro + 1 Macro', c: '· 10 additional micro-creators (₹30,000).\n· 1 macro-creator 100K–500K (₹20,000).\n· A/B test: scripted vs unboxing format.\n· Repurpose top 3 UGCs as paid Meta ads.\n· Brief: Bold, authentic. NO influencer codes.\n· Track: which UGC format drives highest ROAS.', color: COLOR_GREEN },
      { t: 'PHASE 3 — MONTH 3+', sub: '3 Brand Ambassadors', c: '· Select top 3 performers from Phase 1+2.\n· Long-term: 3 months minimum commitment.\n· Early access to new product drops.\n· Exclusive ambassador-only WhatsApp group.\n· Co-created content for YouTube long-form.\n· Community: "Black Ape Athletes" identity badge.', color: COLOR_AMBER }
    ];

    ambCols.forEach((col, i) => {
      let x = 0.5 + (i * 4.25);
      s28.addShape('rect', { x, y: 1.85, w: 3.9, h: 4.8, fill: { color: 'FFFFFF' }, rectRadius: 0.1, line: { color: col.color, width: 2 } });
      s28.addShape('rect', { x, y: 1.85, w: 3.9, h: 0.8, fill: { color: col.color } });
...
function addProduct() {
  prods.push({ id: 'p' + Date.now(), name: 'New Product', mfg_per_pc: 0, variant_type: 'single', extras: [], variants: [{ id: 'v' + Date.now(), name: 'Default', qty: 1, mfgO: null, compO: null }] });
  renderAll(); deferPricingSave();
}
...
function deferPricingSave() {
  ...
  pricingSaveTimer = setTimeout(() => executePricingSave(chip), 1000);
}
...
async function executePricingSave(chip) {
  ...
  try {
    ...
    if (chip) { chip.textContent = '✕ Error'; chip.className = 'chip error'; }
    return false;
  }
}
    ...
    return `
      <div class="catalog-card">
        ...
        <div class="catalog-card-body">
          ${extraSum > 0 ? `<div style="display:flex;flex-wrap:wrap;gap:4px;margin-bottom:4px">${extraBadges}</div>` : ''}
          ${variantsHtml}
        </div>
      </div>`;
  }).join('');
