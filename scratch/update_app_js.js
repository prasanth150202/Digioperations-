...
if (startIndex === -1) {
  console.error("Error: Could not find start of genPPTX()");
  process.exit(1);
}

const endIndex = code.indexOf(endKeyword);
if (endIndex === -1) {
  console.error("Error: Could not find cleanPrice function for end boundary");
  process.exit(1);
}

// Find the closing brace of genPPTX, which should be right before cleanPrice function
const beforeEndIndex = code.lastIndexOf('}', endIndex);
if (beforeEndIndex === -1 || beforeEndIndex < startIndex) {
  console.error("Error: Could not find closing brace of genPPTX");
  process.exit(1);
}

// Extract everything up to start of genPPTX
const beforePart = code.substring(0, startIndex);
// Extract everything after genPPTX
const afterPart = code.substring(beforeEndIndex + 1);

// New dynamic genPPTX code
const newGenPPTX = `async function genPPTX() {
  ...
  try {
    ...
    heroProds.forEach((hp, idx) => {
      ...
      s7.addText(\`\${hp.g} · \text{hp.n}\`, {x: x + 0.15, y: y + 0.1, w: 2.6, fontSize: 11, bold: true, color: 'FFFFFF', fontFace: FONT_PRIMARY, h: 0.35});
...
const finalCode = beforePart + newGenPPTX + afterPart;
fs.writeFileSync(filePath, finalCode, 'utf8');

// Verify syntax (wrapped filePath in double quotes for path space handling)
try {
  execSync('node -c "' + filePath + '"');
  console.log("SUCCESS: app.js syntax is valid and genPPTX() has been updated!");
} catch (err) {
  console.error("Syntax Error after update:", err.message);
  process.exit(1);
}
